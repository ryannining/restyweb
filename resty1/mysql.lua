...
local function fread_result(self, est_nrows)
    fetchnil=function() return nil end
    if self.state ~= STATE_COMMAND_SENT then
        return fetchnil, "cannot read result in the current context: " .. self.state
    end

    local sock = self.sock
    if not sock then
        return fetchnil, "not initialized"
    end
    ...
    if typ == "ERR" then
        ...
        local errno, msg, sqlstate = _parse_err_packet(packet)
        return fetchnil, msg, errno, sqlstate
    end
    ...
    if typ ~= 'DATA' then
        ...
        return fetchnil, "packet type " .. typ .. " not supported"
    end
    ...
    local cols = new_tab(field_count, 0)
    local colsx = new_tab(field_count, 0)
    for i = 1, field_count do
        local col, err, errno, sqlstate = _recv_field_packet(self)
        if not col then
            return nil, err, errno, sqlstate
    ...
    if typ ~= 'EOF' then
        return fetchnil, "unexpected packet type " .. typ .. " while eof packet is "
            .. "expected"
    end
    ...
    function fetchrow()
      ...
      row = _parse_row_data_packet(packet, cols, compact)
    ...
    return fetchrow, colsx
end
_M.fread_result = fread_result
