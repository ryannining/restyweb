...
local interpreter = {
  ...
  frun = function(self,wfilename,rundebug)
    ss=ide.config.path.moonscript
    fnx=wfilename:GetFullPath()
    fnc=''
    if string.find(fnx, ".sass", 1)~=nil then 
      ss=ide.config.path.sassscript 
      fnc=" "..string.gsub(fnx,'.sass','.css').." "
    end
    if string.find(fnx, ".coffee", 1)~=nil then 
      ss=ide.config.path.coffeescript 
    end
      
    exe = ss -- check if the path is configured
    if not exe then
    ...
    local cmd = '"'..ss..'" '..code..fnc..(params and " "..params or "")
...
local spec = {
  ...
  keywords = {
    ...
    [[raw echo query aquery finish assert collectgarbage dofile error getfenv getmetatable ipairs load loadfile loadstring
